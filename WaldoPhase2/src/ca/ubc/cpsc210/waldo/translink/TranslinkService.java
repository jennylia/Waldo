...
public class TranslinkService {
	...
	private final static String APIKEY = "g6akCNjo4BzcQQee3Wln"; 
	...
	public Set<BusStop> getBusStopsAround(LatLon location, int radius) {
		// shows how to read and parse from web service
		...
		InputStream in = null;
		try {
			String s = makeJSONQuery(urlBuilder); //takes urlbuilder calls webservice api at location
			// gives back json as a string
			return parseBusStopsAroundFromJSON(s); // this parses string into json object
		} catch (Exception e) {
	...
	private String makeJSONQuery(StringBuilder urlBuilder) {
		try { //http://kramer.nss.cs.ubc.ca:8080/initsession/
			URL url = new URL("http:" + urlBuilder.toString());
			HttpURLConnection client = (HttpURLConnection) url.openConnection(); //connection
			client.setRequestProperty("accept", "application/json");
			InputStream in = client.getInputStream(); // gets back info
			BufferedReader br = new BufferedReader(new InputStreamReader(in)); //get individual lines
